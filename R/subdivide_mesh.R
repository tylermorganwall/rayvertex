  ...
  if (is.na(id)) {
    for (i in seq_along(scene$shapes)) {
      ...
      if (normals) {
        ...
        scene$shapes[[i]]$has_vertex_normals = rep(
          FALSE,
          nrow(new_shape_info$indices)
        )
      }
    ...
    if (normals) {
      ...
      scene$shapes[[id]]$has_vertex_normals = rep(
        FALSE,
        nrow(new_shape_info$indices)
      )
    }
