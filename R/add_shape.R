...
set_material = function(mesh, 
  ...
  if(!is.null(mesh$materials) && length(mesh$materials) > 0) {
    for(i in seq_len(length(mesh$materials))) {
      ...
      mesh$materials[[i]]$diffuse            = convert_color(diffuse) * diffuse_intensity
      mesh$materials[[i]]$specular           = convert_color(specular) * specular_intensity
      mesh$materials[[i]]$transmittance      = convert_color(transmittance)
      mesh$materials[[i]]$emission           = convert_color(emission) * emission_intensity
    ...
    mesh$materials[[1]]$diffuse            = convert_color(diffuse) * diffuse_intensity
    mesh$materials[[1]]$specular           = convert_color(specular) * specular_intensity
    mesh$materials[[1]]$transmittance      = convert_color(transmittance)
    mesh$materials[[1]]$emission           = convert_color(emission) * emission_intensity
