...
#'                     light_info = directional_light(c(0,1,1)) |>
#'                       add_light(directional_light(c(1,1,-1))))
smooth_normals_mesh = function(mesh, id = NA) {
  ...
  if (is.na(id)) {
    for (i in seq_len(length(mesh$shapes))) {
      ...
      mesh$shapes[[i]]$has_vertex_normals = rep(
        TRUE,
        length(mesh$shapes[[i]]$indices)
      )
    ...
    mesh$shapes[[id]]$has_vertex_normals = rep(
      TRUE,
      length(mesh$shapes[[id]]$indices)
    )
