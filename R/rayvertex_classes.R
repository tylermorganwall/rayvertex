...
#' @keywords internal
ray_shape = function(...) {
  vctrs::new_vctr(list(...), class = c("ray_shape", "list"))
}
...
ray_shape_list = function(...) {
  vctrs::new_vctr(..., class = c("ray_shape_list", "list"))
}
...
convert_rgb_to_ansi = function(color) {
  ...
  rgb_to_ansi256_bg = function(r, g, b) {
    ...
    return(
      16 +
        36 * as.integer(r / 255 * 5) +
        6 * as.integer(g / 255 * 5) +
        as.integer(b / 255 * 5)
    )
  }
...
cat_color = function(
  ...
  if (print_col) {
    colbg = sprintf(
      "%s %s",
      convert_rgb_to_ansi(convert_color(color)),
      "\033[0m"
    )
    intensity_str = ""
    if (intensity != 1) {
      intensity_str = sprintf(
        "%s %0.1f",
        pillar::style_subtle("| intensity:"),
        intensity
      )
    }
    cat(sprintf(
  ...
  print_single_mat = function(x, i = 0) {
    ...
    if (x$ambient_texname != "") {
      ...
      cat(
        sprintf(
          ...
          spacer,
          bullet,
          pillar::style_subtle("ambient_texname:"),
          x$ambient_texname,
          fileexists_str
        ),
        sep = "\n"
    ...
    if (x$specular_texname != "") {
      ...
      cat(
        sprintf(
          ...
          spacer,
          bullet,
          pillar::style_subtle("specular_texname:"),
          x$specular_texname,
          fileexists_str
        ),
        sep = "\n"
    ...
    if (x$emissive_texname != "") {
      ...
      cat(
        sprintf(
          ...
          spacer,
          bullet,
          pillar::style_subtle("emissive_texname:"),
          x$emissive_texname,
          fileexists_str
        ),
        sep = "\n"
    ...
    if (x$bump_texname != "") {
      ...
      cat(
        sprintf(
          ...
          spacer,
          bullet,
          pillar::style_subtle("bump_texname:"),
          x$bump_texname,
          fileexists_str,
          bump_int_str
        ),
    ...
    if (x$normal_texname != "") {
      ...
      cat(
        sprintf(
          ...
          spacer,
          bullet,
          pillar::style_subtle("normal_texname:"),
          x$normal_texname,
          fileexists_str
        ),
        sep = "\n"
    ...
    if (x$dissolve < 1 || any(x$transmittance > 0)) {
      cat(
        sprintf(
          ...
          spacer,
          bullet,
          pillar::style_subtle("translucent:"),
          ifelse(x$translucent, "TRUE", "FALSE")
        ),
        sep = "\n"
      )
    }
    ...
    if (x$type == "toon") {
      cat(
        sprintf(
          "%s%s %s %s",
          pillar::style_subtle("toon_levels:"),
          x$toon_levels
        ),
        sep = "\n"
      )
      cat(
        sprintf(
          "%s%s %s %0.2f",
          pillar::style_subtle("toon_outline_width:"),
          x$toon_outline_width
        ),
        sep = "\n"
      )
...
print.ray_mesh = function(x, ...) {
  ...
  if (length(find.package("tibble", quiet = TRUE)) > 0) {
    withr::local_options(
      list(pillar.advice = TRUE, pillar.print_max = 3, pillar.width = 50),
      code = {
