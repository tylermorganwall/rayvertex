...
sweep_controls = function(
  ...
  if (precomputed) {
    ...
    if (closed && closed_smooth && !straight) {
      ...
      cps = sweep_control_points_straight(points)
    } else {
      cps = sweep_control_points(points)
      if (closed) {
        first = cps[[1]]
        last = cps[[length(cps)]]
...
sweep_curve = function(cps, t, closed) {
  ...
  local = s - index + 1
  point = sweep_bezier(cps[[index]], local)
  tangent = sweep_bezier_deriv(cps[[index]], local)
  unit = function(x) {
    len = sqrt(sum(x^2))
  ...
  if (abs(s - round(s)) < 1e-12 && (s > 0 && s < length(cps) || closed)) {
    ...
    tangent = unit(
      unit(sweep_bezier_deriv(cps[[before]], 1)) +
        unit(sweep_bezier_deriv(cps[[after]], 0))
    )
  }
...
sweep_width = function(width, width_end, ease) {
  ...
  if (utils::tail(values$x, 1) < 1) {
    values$x = c(values$x, 1)
    values$y = c(values$y, utils::tail(values$y, 1))
  }
  if (ease == "spline") {
  ...
  function(u) {
    ...
    t = switch(
      ease,
      linear = t,
      quad = sweep_quad_in_out(t),
      cubic = sweep_cubic_in_out(t),
      exp = (sweep_exp_in_out(t) - sweep_exp_in_out(0)) /
        (sweep_exp_in_out(1) - sweep_exp_in_out(0))
    )
...
sweep_mesh_data = function(
  ...
  if (is.null(initial_normal)) {
    second = sweep_bezier_2nd_deriv(cps[[1]], 0)
  ...
  if (straight) {
    ...
    grid = sort(unique(c(
      grid,
      if (linear_step) distance_at(knots) else knots
    )))
  }
  result = list()
