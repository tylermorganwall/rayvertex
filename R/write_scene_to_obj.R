...
write_scene_to_obj = function(
  ...
  writeLines("# OBJ created with rayvertex", con = con)
  if (materials) {
    con_mtl = file(mtl_file, open = "w")
    on.exit(close(con_mtl), add = TRUE)
    write_materials(con_mtl, scene$materials)
    writeLines(sprintf("mtllib %s", basename(mtl_file)), con = con)
  }
...
write_verts = function(con, verts) {
  ...
  for (i in seq_len(n_verts)) {
    writeLines(
      sprintf("v %f %f %f", verts[[i]][, 1], verts[[i]][, 2], verts[[i]][, 3]),
      con = con
    )
  }
}
...
write_tex = function(con, tex) {
  ...
  for (i in seq_len(n_tex)) {
    if (nrow(tex[[i]] > 0)) {
      writeLines(sprintf("vt %f %f", tex[[i]][, 1], tex[[i]][, 2]), con = con)
    }
  }
}
...
write_norm = function(con, norm) {
  ...
  for (i in seq_len(n_norm)) {
    if (nrow(norm[[i]] > 0)) {
      writeLines(
        sprintf("vn %f %f %f", norm[[i]][, 1], norm[[i]][, 2], norm[[i]][, 3]),
        con = con
      )
    }
  }
...
write_shapes = function(
  ...
  for (i in seq_len(length(shapes))) {
    ...
    prev_mat = 0
    if (
      single_material && single_tex && single_norm && (single_mat || !materials)
    ) {
      ...
      if (!has_tex && !has_norm) {
        face_lines = paste(
          "f",
          apply(shapes[[i]]$indices + voff, 1, paste, collapse = " ")
        )
      ...
      for (j in seq_len(n_idx)) {
        ...
        if (!has_tex && !has_norm) {
          output_list[[counter]] = paste(
            "f",
            apply(shapes[[i]]$indices[j, ] + voff, 1, paste, collapse = " ")
          )
        } else if (has_tex && has_norm) {
          output_list[[counter]] = paste(
            "f",
            paste0(
              shapes[[i]]$indices[j, ] + voff,
              "/",
              shapes[[i]]$tex_indices[j, ] + toff,
              "/",
              shapes[[i]]$norm_indices[j, ] + noff,
              collapse = " "
            )
          )
        } else if (has_tex && !has_norm) {
...
write_materials = function(con, materials) {
  ...
  for (i in seq_len(n_mats)) {
    ...
    if (tmp_mat$diffuse_intensity > 0) {
      writeLines(
        sprintf(
          "Kd %s",
          paste(
            paste_digit(tmp_mat$diffuse * tmp_mat$diffuse_intensity),
            collapse = " "
          )
        ),
        con
    ...
    if (tmp_mat$ambient_intensity > 0) {
      writeLines(
        sprintf(
          "Ka %s",
          paste(
            paste_digit(tmp_mat$ambient * tmp_mat$ambient_intensity),
            collapse = " "
          )
        ),
        con
    ...
    if (tmp_mat$specular_intensity > 0) {
      writeLines(
        sprintf(
          "Ks %s",
          paste(
            paste_digit(tmp_mat$specular * tmp_mat$specular_intensity),
            collapse = " "
          )
        ),
        con
    ...
    writeLines(
      sprintf(
        "Tf %s",
        paste(paste_digit(tmp_mat$transmittance), collapse = " ")
      ),
      con
    )
    if (any(tmp_mat$emission > 0)) {
      writeLines(
        sprintf(
          "Ke %s",
          paste(
            paste_digit(tmp_mat$emission * tmp_mat$emission_intensity),
            collapse = " "
          )
        ),
        con
