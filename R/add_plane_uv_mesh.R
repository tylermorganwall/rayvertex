...
add_plane_uv_mesh = function(
  ...
  if (
    (is.null(direction) || missing(direction)) && !is.null(u) && !is.null(v)
  ) {
    duv[1] = TRUE
  }
  ...
  for (i in seq_along(mesh$shapes)) {
    ...
    vertices_bbox[, 1] = (vertices_bbox[, 1] - xrange[1]) /
      (xrange[2] - xrange[1])
    vertices_bbox[, 3] = (vertices_bbox[, 3] - zrange[1]) /
      (zrange[2] - zrange[1])
...
add_sphere_uv_mesh = function(
  ...
  for (i in seq_along(mesh$shapes)) {
    ...
    vertices_centered = vertices -
      matrix(origin, nrow = nrow(vertices), ncol = 3, byrow = TRUE)
...
#'
#' @keywords internal
generate_rotation_matrix_from_direction = function(
  direction = c(0, 1, 0),
  up = c(0, 1, 0)
) {
