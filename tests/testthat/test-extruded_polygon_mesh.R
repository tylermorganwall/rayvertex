...
polygon_test_mesh = function(...) {
  mesh = extruded_polygon_mesh(...)
  list(vertices = mesh$vertices[[1]], indices = mesh$shapes[[1]]$indices)
}
...
test_that("large translations and centering preserve geometry and orientation", {
  ...
  object = extruded_polygon_mesh(p, position = c(5, 2, -3))
  expect_equal(object$vertices[[1]], sweep(base$vertices, 2, c(5, 2, -3), "+"))
})
...
test_that("invalid polygon inputs fail before triangulation", {
  ...
  for (holes in list(
    ...
    expect_error(
      extruded_polygon_mesh(rbind(p * 5, p + 1, p + 2), holes = holes),
      "holes"
    )
  }
  ...
  for (scale in list(0, c(1, 0, 1), c(1, 2), Inf, NA, numeric(), "one")) {
    expect_error(extruded_polygon_mesh(p, scale = scale), "scale")
  }
  for (plane in list("x", NA, 1, c("xy", "xz"))) {
    expect_error(extruded_polygon_mesh(p, plane = plane), "plane")
  }
  for (flag in c("center", "flip_horizontal", "flip_vertical")) {
    expect_error(
      do.call(
        extruded_polygon_mesh,
        c(list(polygon = p), setNames(list(NA), flag))
      ),
      flag
    )
  }
  for (ring in list(
    ...
    expect_error(extruded_polygon_mesh(ring), "vertices|coordinates")
  }
  expect_error(extruded_polygon_mesh(cbind(0:3, 0)), "area|retrace")
  expect_error(extruded_polygon_mesh(p[c(1, 3, 2, 4), ]), "self-intersect")
  # Nonzero signed area does not make a self-intersecting ring valid.
  expect_error(
    extruded_polygon_mesh(rbind(c(0, 0), c(4, 0), c(0, 3), c(3, 3), c(1, -1))),
    "self-intersect"
  )
  expect_error(extruded_polygon_mesh(rbind(p * 5, p + 20), holes = 5), "inside")
  expect_error(extruded_polygon_mesh(rbind(p * 5, p), holes = 5), "touch")
  expect_error(
    extruded_polygon_mesh(rbind(p * 5, p + 1, p + 1.5), holes = c(5, 9)),
    "cross"
  )
  expect_error(
    extruded_polygon_mesh(
      rbind(p * 5, p * 2 + 1, p * .5 + 1.5),
      holes = c(5, 9)
    ),
    "contain"
  )
  expect_error(extruded_polygon_mesh(p * 1e-200, scale = 1e-200), "collapsed")
})
...
test_that("sf preserves multipart holes, heights and XYZ geometry", {
  ...
  for (g in list(
    ...
    expect_error(
      extruded_polygon_mesh(sf::st_sf(geometry = sf::st_sfc(g))),
      "Empty|vertices|POLYGON"
    )
  }
  expect_error(
    extruded_polygon_mesh(sf::st_sf(geometry = sf::st_sfc())),
    "Empty"
  )
})
...
test_that("sf extrusion decisions use each resolved height pair", {
  ...
  for (values in list(c(0, NA, 1), c(0, Inf, 1), c("a", "b", "c"))) {
    s$bad = values
    expect_error(
      extruded_polygon_mesh(s, data_column_top = "bad"),
      "finite numeric height"
    )
  }
  expect_warning(
    extruded_polygon_mesh(s, data_column_bottom = "missing"),
    "data_column_bottom"
  )
  expect_error(
    extruded_polygon_mesh(s, data_column_top = c("high", "low")),
    "single column name"
  )
})
