...
test_that("periodic control points have matching first and second derivatives", {
  ...
  for (i in seq_along(cps)) {
    ...
    expect_equal(
      sweep_bezier_deriv(cps[[i]], 1),
      sweep_bezier_deriv(cps[[next_i]], 0)
    )
    expect_equal(
      sweep_bezier_2nd_deriv(cps[[i]], 1),
      sweep_bezier_2nd_deriv(cps[[next_i]], 0)
    )
  }
})
...
test_that("zero-width tips use triangle fans with no degenerate triangles", {
  ...
  expect_null(extruded_path_mesh(sweep_test_line(), width = 0, breaks = 5))
})
