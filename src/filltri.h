...
void fill_tri_blocks(std::vector<int> block_faces,
                     ...
                     rayimage& position_buffer) { 
  for(int entry=0; entry < block_faces.size(); entry++) {
    int face = block_faces[entry];
    
    vec3 v1 = ndc_verts[0][face] * ndc_inv_w[0][face];
    vec3 v2 = ndc_verts[1][face] * ndc_inv_w[1][face];
    vec3 v3 = ndc_verts[2][face] * ndc_inv_w[2][face];
    
    float v1_ndc_inv_w = ndc_inv_w[0][face];
    float v2_ndc_inv_w = ndc_inv_w[1][face];
    float v3_ndc_inv_w = ndc_inv_w[2][face];
      ...
      for (uint32_t i = xmin; i < xmax; i++) {
        ...
        for (uint32_t j = ymin; j < ymax; j++) {
          ...
          if (w1 >= 0 && w2 >= 0 && w3 >= 0) {
            ...
            float z = v1.z * bc_clip.x + v2.z * bc_clip.y + v3.z * bc_clip.z;
            if(z > zbuffer(i,j)) continue;
            bool discard = shader->fragment(bc_clip, color, position, normal, face);
