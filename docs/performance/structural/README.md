...
All 245 corrected clipping reference outputs match exactly. The source suite passes, including output-demand allocation checks and 127×91 parallel background/outline comparisons. The standalone arena ASan/UBSan test also covers all eight auxiliary masks and early release. `RAYVERTEX_REFERENCE_BUFFERS=1` restores full allocation for developer checks; `RAYVERTEX_REFERENCE_SCREEN=1` keeps screen passes serial. [Measured results](demand/results.md) include substantial layered-transparency time/memory reductions and retained toon/environment public-call regressions.
...
This path is opt-in with `RAYVERTEX_VISIBILITY=1`; it remains disabled by default. [Measured results](visibility/results.md) show a large back-to-front overdraw benefit and little small-scene change, alongside other regressions. Both default and opt-in paths pass all 245 clipped-scalar comparisons. Source tests pass, including winner shader-call bounds, mixed-alpha parity and exceptional-opacity replay. Diagnostic coverage/shading times are sums of tile worker durations; they are not additive wall times on multiple workers. Clocks disappear from uninstrumented kernels. The new `overdraw` fixture submits the same 16-layer Phong grid back-to-front; the existing `occluded` fixture retains front-to-back order so early-Z benefits are not conflated.


## Prepared scene snapshots

`f44b9b6` adds the documented opt-in `prepare_scene()` / `update_prepared_scene()` API. Geometry is validated, merged, deduplicated and snapshotted once; material images are decoded into immutable shared owners. The handle is session/process-local and has an external-pointer finalizer. R copy-on-modify ownership protects the retained snapshot, including repeated FSAA and callback use. Every frame still evaluates camera/light/effects/callback inputs and rebuilds shadows/environment variants. [Timings, RSS, preparation/first-call costs and reuse estimates](prepared/results.md) are separate from ordinary and native regimes. Both ordinary and prepared 245-result corpora are exact; ownership tests cover GC, partial decode failure, file changes, original-list mutation and invalid handles.

## Exact shadow and light specialization

`5f0853f` prepares directional vectors once, preserves all 25 PCF taps with interior scalar-depth loads and a clamped border path, skips empty transparent shadow reads only after a conservative barrier-published flag, and shades proven-opaque shadow casters with the same depth arithmetic. Alpha-dependent casters retain the generic path. [Results](shadows/results.md) record the sphere-shadow benefit and an alpha16 regression.

## Shared environment variants and light samples

`fbe4359` shares exact environment resize variants by integer dimensions and immutable source. Unblurred materials share the original; background blur receives its own variant. Point-light calculations reuse displacement/distance/direction/attenuation without algebraic reassociation or changing the legacy exceptional zero-distance path. Main and shadow pass templates remove shadow-only decisions from the color loop. [Results](assets/results.md) record exact comparisons, variant counts, targeted RSS reductions and the disappearance of the preceding alpha16 regression. Standalone ASan/UBSan checks compare 100,000 light samples exactly with the scalar functions.
